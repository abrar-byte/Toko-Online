...
export default class Beli extends Component {
  ...
  masuk = (i) => {
    ...
    localStorage.setItem("detail", JSON.stringify(this.state.produk.data.
      resultValue[
      "icms-zebra-5000383-2586219"].data[i]))

  }

  render() {
    console.log(this.state.produk.data.
      resultValue[
                ...
                {produk.map((item, i) =>
                  <Col key={i} lg={3} md={4} sm={6} >
                    <Card className="min-vh-100 my-3" bg="light" text="dark" >
                      ...
                      <Link className="d-flex justify-content-center" to="/detail"><Button onClick={() => this.masuk(i)/* () => localStorage.setItem('detail', JSON.stringify(produk[i])) */}>Check In</Button></Link>
                    </Card>
                  </Col>
                )}
              </Row>
